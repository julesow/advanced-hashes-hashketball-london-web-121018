...
def team_colors(team_name)
  team_name =team_name.to_sym
    ...
    if game_hash[:home][:team_name] == "Brooklyn Nets"
      game_hash[:home][:team_name][:colors]
    else
 ...
 def team_names(game_hash)
    ...
    game_hash.each do |loc,data|
     data.each do |attribute,team_name|
      puts game_hash[loc{team_name}
   end
  end 
...
# knows the number of points scored by each player
  if game_hash[:home][:players].include?(player_name)
    game_hash[:home][:players][player_name][:numbers]
    elsif game_hash[:away][:players].include?(player_name)
    game_hash[:away][:players][player_name][:numbers]
  else
    puts "No #{player_name} in the players list"
end
end
